...
USE [SYSADMINDB] -- Specify the database in which the objects will be created.

SET NOCOUNT ON

DECLARE @CreateJobs nvarchar(max)          = 'N'						-- Specify whether jobs should be created.
DECLARE @BackupDirectory nvarchar(max)     = 'F:\Copy-Only-Backup'      -- Specify the backup root directory. If no directory is specified, the default backup directory is used.
...
DECLARE @OutputFileDirectory nvarchar(max) = 'F:\Copy-Only-Backup'      -- Specify the output file directory. If no directory is specified, then the SQL Server error log directory is used.
